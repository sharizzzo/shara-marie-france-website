...
- **The 700px mobile breakpoint isn't the only one that matters — audit the ~700-1024px tablet range separately.** A responsive audit at 375/768/1024px (2026-08-27) found three bugs that a mobile-only (≤700px) check had missed, because the elements in question are only *problematic*, not present/hidden, above 700px:
  - `.fw-connector-merge` (the Framework section's SVG merge-connector) had `box-sizing: content-box` with `width: 100%; padding: 0 48px` — content-box adds padding *outside* the 100% width, overflowing the page by 96px at any width below ~996px where the connector isn't yet hidden by the `.fw-connector-merge--desktop{display:none}` mobile rule. Fixed by switching it to `border-box` (the site-wide default everywhere else).
  - `.review-card`'s fixed `600px` left panel (the Alignment Review callout) only had a stacking fallback below 700px, but 600px plus a usable right-panel width doesn't fit until roughly 1000px. It, and the header/nav wrap rules (same problem — "Shara Marie-France" and "About Me" were wrapping mid-phrase inside squeezed flex items instead of the header wrapping to two clean rows), now live in their own `@media (max-width: 1000px)` block instead of the 700px one.
  - **A `scrollWidth > clientWidth` check alone won't catch this class of bug** — flexbox/text shrinking below content size means things wrap awkwardly *inside* their box rather than overflowing the page. Only a visual check (screenshot) surfaces it.
  - `.nav-link` had no padding — a ~15px-tall tap target, well under the ~44px touch-target minimum. Fixed with `padding: 14px 0` through `@media (max-width: 1024px)` (desktop above that keeps the original tight nav spacing on purpose).
